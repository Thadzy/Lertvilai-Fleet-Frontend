...
function rw() {
	...
	return ki.read = function(t, e, r, n, i) {
		...
		if (s === 0) s = 1 - f;
			...
			if (s === c) return o ? NaN : (y ? -1 : 1) * Infinity;
		...
		for (e = Math.abs(e), isNaN(e) || e === Infinity ? (a = isNaN(e) ? 1 : 0, o = h) : (o = Math.floor(Math.log(e) / Math.LN2), e * (c = Math.pow(2, -o)) < 1 && (o--, c *= 2), o + d >= 1 ? e += w / c : e += w * Math.pow(2, 1 - d), e * c >= 2 && (o++, c /= 2), o + d >= h ? (a = 0, o = h) : o + d >= 1 ? (a = (e * c - 1) * Math.pow(2, i), o = o + d) : (a = e * Math.pow(2, d - 1) * Math.pow(2, i), o = 0)); i >= 8; t[r + y] = a & 255, y += P, a /= 256, i -= 8);
...
function If() {
	...
	function R(p, u, l) {
		...
		for (p || (p = "utf8");;) switch (p) {
			...
			case "ascii": return X(this, u, l);
			case "latin1":
			case "binary": return Q(this, u, l);
	...
	o.prototype.write = function(u, l, S, C) {
		...
		let F = !1;
		for (;;) switch (C) {
			case "hex": return _(this, u, l, S);
			case "utf8":
			case "utf-8": return q(this, u, l, S);
			case "ascii":
			case "latin1":
			case "binary": return $(this, u, l, S);
			case "base64": return j(this, u, l, S);
			case "ucs2":
			case "ucs-2":
			case "utf16le":
			case "utf-16le": return J(this, u, l, S);
			default:
			...
			if (C + k <= l) {
				...
				switch (k) {
					...
					case 3:
						...
						break;
					case 4: B = p[C + 1], re = p[C + 2], ie = p[C + 3], (B & 192) === 128 && (re & 192) === 128 && (ie & 192) === 128 && (oe = (N & 15) << 18 | (B & 63) << 12 | (re & 63) << 6 | ie & 63, oe > 65535 && oe < 1114112 && (F = oe));
				}
			}
...
function yw() {
	...
	s.prototype.getMaxListeners = function() {
		...
		if (q) {
			...
			var J = /* @__PURE__ */ new Error("Unhandled error." + (j ? " (" + j.message + ")" : ""));
			throw J.context = j, J;
		}
		var W = $[g];
	...
	function f(T, g, O, _) {
		...
		if (a(O), $ = T._events, $ === void 0 ? ($ = T._events = Object.create(null), T._eventsCount = 0) : ($.newListener !== void 0 && (T.emit("newListener", g, O.listener ? O.listener : O), $ = T._events), j = $[g]), j === void 0) j = $[g] = O, ++T._eventsCount;
			...
			j.warned = !0;
			var J = /* @__PURE__ */ new Error("Possible EventEmitter memory leak detected. " + j.length + " " + String(g) + " listeners added. Use emitter.setMaxListeners() to increase limit");
...
var Xr = D((DO, Vf) => {
	...
	var em = et((t, e, r) => {
		if (!Df(r, t)) throw new Jr(e, t, "must be one of: " + jf(Bw(r, (s) => typeof s == "string" ? `'${s}'` : Dw(s)), ", "));
	});
	function $f(t, e) {
...
var Wd = D((bC, Fd) => {
	...
	Fd.exports = function t(e, r) {
		...
		if (typeof e?.writable == "object" || typeof e?.readable == "object") return an({
			readable: e != null && e.readable ? Od(e?.readable) ? e?.readable : t(e.readable) : void 0,
			writable: e != null && e.writable ? Rd(e?.writable) ? e?.writable : t(e.writable) : void 0
		});
		let n = e?.then;
...
var gp = D((EB, va) => {
	...
	function co(t, e) {
		...
		return e?.highWaterMark != null && (n = up(e.highWaterMark)), ap(r, "options.concurrency", 1), ap(n, "options.highWaterMark", 0), n += r, async function* () {
			let s = $e().AbortSignalAny([e?.signal].filter(nS)), o = this, a = [], c = { signal: s }, f, h, d = !1, w = 0;
			function y() {
				d = !0, P();
			}
			function P() {
...
var Up = D((y2, Lp) => {
	...
	function DS(t) {
		...
		function r(h) {
			...
			function E(...b) {
				if (!E.enabled) return;
				let m = E, v = Number(/* @__PURE__ */ new Date());
				m.diff = v - (d || v), m.prev = d, m.curr = v, d = v, b[0] = r.coerce(b[0]), typeof b[0] != "string" && b.unshift("%O");
				let T = 0;
...
var Fp = D((M2, jp) => {
	...
	jp.exports = class t extends zS {
		...
		_parseConnect() {
			...
			if (o.clean = (this._list.readUInt8(this._pos) & de.CLEAN_SESSION_MASK) !== 0, this._pos++, o.keepalive = this._parseNum(), o.keepalive === -1) return this._emitError(/* @__PURE__ */ new Error("Packet too short"));
		...
		_parseSubscribe() {
			...
			if (e.subscriptions = [], !!this._parseMessageId()) {
				...
				for (; this._pos < e.length;) {
					...
					if (i = n & de.SUBSCRIBE_OPTIONS_QOS_MASK, i > 2) return this._emitError(/* @__PURE__ */ new Error("Invalid subscribe QoS, must be <= 2"));
					if (a = (n >> de.SUBSCRIBE_OPTIONS_NL_SHIFT & de.SUBSCRIBE_OPTIONS_NL_MASK) !== 0, o = (n >> de.SUBSCRIBE_OPTIONS_RAP_SHIFT & de.SUBSCRIBE_OPTIONS_RAP_MASK) !== 0, s = n >> de.SUBSCRIBE_OPTIONS_RH_SHIFT & de.SUBSCRIBE_OPTIONS_RH_MASK, s > 2) return this._emitError(/* @__PURE__ */ new Error("Invalid retain handling, must be <= 2"));
					c = {
						topic: r,
						qos: i
		...
		_parseSuback() {
			...
			if (this.packet.granted = [], !!this._parseMessageId()) {
				...
				for (; this._pos < this.packet.length;) {
					let r = this._list.readUInt8(this._pos++);
					if (this.settings.protocolVersion === 5) {
						if (!de.MQTT5_SUBACK_CODES[r]) return this._emitError(/* @__PURE__ */ new Error("Invalid suback code"));
		...
		_parseUnsuback() {
			...
			if (this.settings.protocolVersion === 5) {
				...
				for (Object.getOwnPropertyNames(r).length && (e.properties = r), e.granted = []; this._pos < this.packet.length;) {
					...
					if (!de.MQTT5_UNSUBACK_CODES[n]) return this._emitError(/* @__PURE__ */ new Error("Invalid unsuback code"));
			...
			if (this._parseMessageId(), this.settings.protocolVersion === 5) {
				if (e.length > 2) {
					switch (e.reasonCode = this._parseByte(), this.packet.cmd) {
						case "puback":
						case "pubrec":
							if (!de.MQTT5_PUBACK_PUBREC_CODES[e.reasonCode]) return this._emitError(/* @__PURE__ */ new Error("Invalid " + this.packet.cmd + " reason code"));
							break;
						case "pubrel":
						case "pubcomp":
							if (!de.MQTT5_PUBREL_PUBCOMP_CODES[e.reasonCode]) return this._emitError(/* @__PURE__ */ new Error("Invalid " + this.packet.cmd + " reason code"));
							break;
					}
		...
		_parseDisconnect() {
			...
			if (ae("_parseDisconnect"), this.settings.protocolVersion === 5) {
				this._list.length > 0 ? (e.reasonCode = this._parseByte(), de.MQTT5_DISCONNECT_CODES[e.reasonCode] || this._emitError(/* @__PURE__ */ new Error("Invalid disconnect reason code"))) : e.reasonCode = 0;
				let r = this._parseProperties();
				Object.getOwnPropertyNames(r).length && (e.properties = r);
			}
		...
		_parseAuth() {
			...
			if (e.reasonCode = this._parseByte(), !de.MQTT5_AUTH_CODES[e.reasonCode]) return this._emitError(/* @__PURE__ */ new Error("Invalid auth reason code"));
			let r = this._parseProperties();
...
var Gp = D((eN, Ra) => {
	...
	function ZS(t, e, r, n) {
		...
		switch (i) {
			case 0:
			case 1: return M.nextTick(t);
			case 2: return M.nextTick(function() {
				t.call(null, e);
			});
			case 3: return M.nextTick(function() {
				t.call(null, e, r);
			});
			case 4: return M.nextTick(function() {
				t.call(null, e, r, n);
			});
			default:
...
var Ba = D((cN, ey) => {
	...
	x();
	I();
	var ue = Pa(), { Buffer: se } = (ke(), me(qe)), eE = se.allocUnsafe(0), tE = se.from([0]), Vn = Vp(), rE = Gp().nextTick, rt = At()("mqtt-packet:writeToStream"), yo = Vn.cache, nE = Vn.generateNumber, iE = Vn.generateCache, ka = Vn.genBufVariableByteInt, oE = Vn.generate4ByteBuffer, He = Ca, go = !0;
	function Xp(t, e, r) {
	...
	function aE(t, e, r) {
		...
		if (y += i.length + 2, s !== 3 && s !== 4 && s !== 5) return e.destroy(/* @__PURE__ */ new Error("Invalid protocol version")), !1;
	...
	function fE(t, e, r) {
		...
		if (typeof a == "object" && a.length) for (let w = 0; w < a.length; w += 1) {
			...
			if (typeof y != "string") return e.destroy(/* @__PURE__ */ new Error("Invalid subscriptions - invalid topic")), !1;
			if (typeof P != "number") return e.destroy(/* @__PURE__ */ new Error("Invalid subscriptions - invalid qos")), !1;
			if (n === 5) {
				if (typeof (a[w].nl || !1) != "boolean") return e.destroy(/* @__PURE__ */ new Error("Invalid subscriptions - invalid No Local")), !1;
				if (typeof (a[w].rap || !1) != "boolean") return e.destroy(/* @__PURE__ */ new Error("Invalid subscriptions - invalid Retain as Published")), !1;
				let m = a[w].rh || 0;
				if (typeof m != "number" || m > 2) return e.destroy(/* @__PURE__ */ new Error("Invalid subscriptions - invalid Retain Handling")), !1;
			}
	...
	function dE(t, e, r) {
		...
		if (f += 2, typeof a == "object" && a.length) for (let w = 0; w < a.length; w += 1) {
			if (typeof a[w] != "string") return e.destroy(/* @__PURE__ */ new Error("Invalid unsubscriptions")), !1;
	...
	function pE(t, e, r) {
		...
		if (n === 5) if (typeof a == "object" && a.length) for (let y = 0; y < a.length; y += 1) {
			if (typeof a[y] != "number") return e.destroy(/* @__PURE__ */ new Error("Invalid qos vector")), !1;
			d += 1;
		}
		else return e.destroy(/* @__PURE__ */ new Error("Invalid qos vector")), !1;
		let w = null;
		...
		switch (t.log("handlePublish: qos %d", s), s) {
			case 2:
				a.customHandleAcks(n, i, e, (c, f) => {
					...
					if (fy.indexOf(f) === -1) return t.emit("error", /* @__PURE__ */ new Error("Wrong reason code for pubrec"));
					f ? t._sendPacket({
						cmd: "pubrec",
						messageId: o,
						reasonCode: f
			...
			case 1:
				a.customHandleAcks(n, i, e, (c, f) => {
					if (typeof c == "number" && (f = c, c = null), c) return t.emit("error", c);
					if (fy.indexOf(f) === -1) return t.emit("error", /* @__PURE__ */ new Error("Wrong reason code for puback"));
		...
		*rvalues() {
		...
		set(e, r, n = {}) {
			...
			if (d === void 0) d = this.#i === 0 ? this.#o : this.#w.length !== 0 ? this.#w.pop() : this.#i === this.#u ? this.#B(!1) : this.#i, this.#r[d] = e, this.#e[d] = r, this.#n.set(e, d), this.#l[this.#o] = d, this.#h[d] = this.#o, this.#o = d, this.#i++, this.#C(d, h, c), c && (c.set = "add"), f = !1;
				...
				if (r !== w) {
					if (this.#x && this.#t(w)) {
						w.__abortController.abort(/* @__PURE__ */ new Error("replaced"));
...
var Ay = D((Ao) => {
	...
	var jE = It(), Qa = class extends jE.Base {
		constructor(e = [], r = function(i$1, s) {
			return i$1 > s ? -1 : i$1 < s ? 1 : 0;
		}, n = !0) {
...
var mg = D((Ho, wg) => {
	...
	I();
	(function(t, e) {
		typeof Ho == "object" && typeof wg < "u" ? e(Ho, Zy(), sg(), Pu(), lg(), bg()) : typeof define == "function" && define.amd ? define([
			"exports",
		...
		], e) : (t = typeof globalThis < "u" ? globalThis : t || self, e(t.brokerFactory = {}, t._defineProperty, t._slicedToArray, t.fastUniqueNumbers, t._asyncToGenerator, t._regeneratorRuntime));
					...
					return i(s.mark(function g() {
						...
						return s.wrap(function(j) {
							for (;;) switch (j.prev = j.next) {
								case 0: return O = new MessageChannel(), _ = O.port1, q = O.port2, j.next = 1, T("connect", { port: _ }, [_]);
								case 1: return $ = j.sent, a.set(q, $), j.abrupt("return", q);
								case 2:
								case "end": return j.stop();
							}
						}, g);
					}));
				},
						...
						var g = i(s.mark(function O(_) {
							...
							return s.wrap(function($) {
								for (;;) switch ($.prev = $.next) {
									...
									case 1: return $.next = 2, T("disconnect", { portId: q });
									case 2:
...
var vg = D((Vo, _g) => {
	...
	I();
	(function(t, e) {
		typeof Vo == "object" && typeof _g < "u" ? e(Vo, Xn(), mg(), Pu()) : typeof define == "function" && define.amd ? define([
			"exports",
			"@babel/runtime/helpers/typeof",
			"broker-factory",
			"fast-unique-numbers"
		], e) : (t = typeof globalThis < "u" ? globalThis : t || self, e(t.workerTimersBroker = {}, t._typeof, t.brokerFactory, t.fastUniqueNumbers));
	})(Vo, function(t, e, r, n) {
...
var Og = D((Gt) => {
	...
	var xg = Eg(), Lu = LA(yi()), Tg = {
		...
		switch (t) {
			case "native": return Pg;
			case "worker": return Tg;
			case "auto":
			default: return Lu.default && !Lu.isWebWorker && !Lu.isReactNativeBrowser ? Tg : Pg;
		}
	};
	Gt.default = UA;
});
...
var zo = D((yt) => {
	...
	yt.default = class t extends KA.TypedEventEmitter {
		...
		subscribe(e, r, n) {
			...
			if (a !== null) return Fu(n, /* @__PURE__ */ new Error(`Invalid topic ${a}`)), this;
		...
		unsubscribe(e, r, n) {
			...
			if (i !== null) return Fu(n, /* @__PURE__ */ new Error(`Invalid topic ${i}`)), this;
...
function xx() {
	...
	var i = {
		...
		relative: function(o, a) {
			...
			for (var y = a.length - d, P = h < y ? h : y, E = -1, b = 0; b <= P; ++b) {
				if (b === P) {
					if (y > P) {
				...
				var m = o.charCodeAt(c + b);
				if (m !== a.charCodeAt(d + b)) break;
				m === 47 && (E = b);
			}
			var R = "";
...
function Px() {
	...
	El = function k(B, re, ie, oe) {
		...
		if (Re(ce, "maxStringLength") && (typeof ce.maxStringLength == "number" ? ce.maxStringLength < 0 && ce.maxStringLength !== Infinity : ce.maxStringLength !== null)) throw new TypeError("option \"maxStringLength\", if provided, must be a positive integer, Infinity, or `null`");
		...
		if (!fe(B) && !ge(B)) {
			var is = F(B, ut), tc = Q ? Q(B) === Object.prototype : B instanceof Object || B.constructor === Object, os = B instanceof Object ? "" : "null prototype", rc = !tc && K && Object(B) === B && K in B ? v.call(Ne(B), 8, -1) : os ? "Object" : "", ss = (tc || typeof B.constructor != "function" ? "" : B.constructor.name ? B.constructor.name + " " : "") + (rc || os ? "[" + q.call(_.call([], rc || [], os || []), ": ") + "] " : "");
	...
	function at(k, B) {
		...
		return pe(R.call(R.call(k, /(['\\])/g, "\\$1"), /[\x00-\x1f]/g, Zo), "single", B);
	}
	function Zo(k) {
...
function Rx() {
	...
	var t = Ox(), e = vb(), r = Cl(), n = Object.prototype.hasOwnProperty, i = {
		...
		if (d(V) || e.isBuffer(V)) {
			if (q) return [G(K ? m : q(m, h.encoder, X, "key", H)) + "=" + G(q(V, h.encoder, X, "value", H))];
...
function kx() {
	...
	var t = vb(), e = Object.prototype.hasOwnProperty, r = Array.isArray, n = {
		...
		var m = P.parameterLimit === Infinity ? void 0 : P.parameterLimit, v = b.split(P.delimiter, m), R = -1, T, g = P.charset;
		...
		return {
			allowDots: typeof y.allowDots > "u" ? y.decodeDotInKeys === !0 ? !0 : n.allowDots : !!y.allowDots,
...
`,
	...
	e.prototype.parse = function(g, O, _) {
		...
		var q = g.indexOf("?"), $ = q !== -1 && q < g.indexOf("#") ? "?" : "#", j = g.split($);
		j[0] = j[0].replace(/\\/g, "/"), g = j.join($);
		var W = g;
		...
		if (!P[G] && (X || G && !E[G])) {
			...
			this.host = (this.hostname || "") + xe, this.href += this.host, pe && (this.hostname = this.hostname.substr(1, this.hostname.length - 2), W[0] !== "/" && (W = "/" + W));
		}
	...
	return e.prototype.resolveObject = function(g) {
		...
		if (g.protocol && g.protocol !== _.protocol) {
			...
			if (_.search = g.search, _.query = g.query, _.host = g.host || "", _.auth = g.auth, _.hostname = g.hostname || g.host, _.port = g.port, _.pathname || _.search) _.path = (_.pathname || "") + (_.search || "");
...
var _i = D((gr) => {
	...
	var uT = (t, e) => {
		...
		function y(m) {
			...
			let v = /* @__PURE__ */ new Error("WebSocket error");
...
var Ql = D((we) => {
	...
	we.DefaultMessageIdProvider = Ai(Au()).default;
	we.UniqueMessageIdProvider = Ai(Ng()).default;
	we.Store = Ai(La()).default;
	var Gb = WT(Wb());
	...
	we.KeepaliveManager = Ai(Du()).default;
